...
| --- | --- | --- | --- |
| RFSoC SSH private key | `~/.ssh/id_ed25519_rfsoc` | `/home/iccls2/.ssh/id_ed25519_rfsoc` | Yes, if RFSoC PL/XRT SSH status is used |
| RFSoC SSH public key | `~/.ssh/id_ed25519_rfsoc.pub` | `/home/iccls2/.ssh/id_ed25519_rfsoc.pub` | Optional but useful |
| OpenWrt AP SSH private key | `~/.ssh/openwrt_ap_ed25519` | `/home/iccls2/.ssh/openwrt_ap_ed25519` | Yes, if AP SSH gateway is used |
| OpenWrt AP SSH public key | `~/.ssh/openwrt_ap_ed25519.pub` | `/home/iccls2/.ssh/openwrt_ap_ed25519.pub` | Optional but useful |
| SSH config aliases | `~/.ssh/config` | `/home/iccls2/.ssh/config` | Recommended |
| kubeconfig | `~/.kube/config` | old `/home/iccls2/.kube/config` only as reference | Yes, but regenerate for the new k3s cluster |
| Helm repo config | `~/.config/helm/repositories.yaml` | `/home/iccls2/.config/helm/repositories.yaml` | Optional; can be recreated with `helm repo add` |
...
```bash
cat > /home/iccls2/AutoScale/systemd/autoscale-api.env <<'EOF'
AUTOSCALE_API_TOKEN=replace-with-a-real-long-random-token
EOF
chmod 600 /home/iccls2/AutoScale/systemd/autoscale-api.env
```
...
- `thermal-yolo/yolo26_k8s/Dockerfile`
- `thermal-yolo/yolo26_k8s/app.py`
...
```bash
ssh <new-worker> 'test -f /home/icclz1/gpu-tempctl-lab/fan_control_lab/gpu_cycle_runner.py && echo OK'
