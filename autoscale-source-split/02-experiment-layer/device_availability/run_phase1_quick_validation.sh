...
RUN_ID="${1:-phase1_quick_validation_$(date +%Y%m%d_%H%M%S)}"
...
echo "[INFO] This profile follows the currently reviewed quick-validation ladder."
echo "[INFO] Total planned duration is 150 minutes (2h30m), based on the approved phase table."
...
EXPERIMENT_PROFILE=phase1_quick_validation \
...
if [[ "${STRESS_EXIT_CODE}" -eq 0 && "${PROBE_EXIT_CODE}" -eq 0 ]]; then
  echo "[INFO] phase1 quick validation completed"
else
  echo "[WARN] phase1 quick validation finished with non-zero exit codes: stress=${STRESS_EXIT_CODE} probe=${PROBE_EXIT_CODE}"
fi
