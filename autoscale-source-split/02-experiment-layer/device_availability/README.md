...
目前 probe 的 confirmed-outage 定義如下：

- `Node Ready=False`：單一 sample 立即記為 `DOWN`。
- `/healthz` timeout / non-200：先記為 `DEGRADED`；預設連續 `3` 次失敗才升級為 `DOWN`，reason 為 `sentinel_confirmed_outage`。
- `/compute-check` timeout / non-200，以及 `/compute-check latency >= 2s`：記為 `DEGRADED`，不直接計入 `DOWN`。

因此 availability 的分子只扣除 confirmed `DOWN` sample；`DEGRADED`、metrics 缺失與 compute tail 需另行由 rate／reason 統計解讀。
...
```text
A_device = (N_total - N_down) / N_total * 100%

`N_down` 是 effective state 為 `DOWN` 的 sample 數。`summary.json` 的 `total_down_seconds` 以 `N_down * sampling_interval_seconds` 估算；`observation_duration_s` 則由首末 sample timestamp 計算。
```
...
```text
timestamp,phase,node,ready,healthz_ok,compute_ok,metrics_ok,healthz_ms,compute_ms,health_failure_streak,compute_failure_streak,sample_reason,anomaly_reason,is_transient_anomaly,state,downtime_reason
```
...
- `samples_total` / `samples_down` / `samples_degraded`
- `samples_transient_anomaly`
- `observation_start` / `observation_end` / `observation_duration_s`
- `total_observation_seconds`
- `total_down_seconds`
- `availability_percent`
- `confirmed_outage_availability_percent`
- `downtime_reason_breakdown`
- `degraded_per_1000_samples`
