...
IMAGE_PATH=$(find "$HOME/AutoScale" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) | head -n 1)
...
$PY experiments/thermal_analysis/yolo26_latency_client_stable.py \
...
for name in files:
    ...
    if "success" in df.columns:
        ...
        ok = pd.Series([False] * total)

    success = int(ok.sum())
    fail = int(total - success)

    lat = pd.to_numeric(df.loc[ok, "latency_ms_client"], errors="coerce").dropna() if "latency_ms_client" in df.columns else pd.Series(dtype=float)
    srv = pd.to_numeric(df.loc[ok, "server_latency_ms"], errors="coerce").dropna() if "server_latency_ms" in df.columns else pd.Series(dtype=float)
    ...
    rows.append({
        ...
        "client_mean_ms": round(float(lat.mean()), 3) if len(lat) else float("nan"),
        "client_p50_ms": round(q(lat, 0.50), 3),
        "client_p95_ms": round(q(lat, 0.95), 3),
        "client_p99_ms": round(q(lat, 0.99), 3),
        "client_max_ms": round(float(lat.max()), 3) if len(lat) else float("nan"),
        "server_mean_ms": round(float(srv.mean()), 3) if len(srv) else float("nan"),
        "server_p95_ms": round(q(srv, 0.95), 3),
        "server_p99_ms": round(q(srv, 0.99), 3),
        "server_max_ms": round(float(srv.max()), 3) if len(srv) else float("nan"),
        "client_latency_gt_1s": int((lat > 1000).sum()) if len(lat) else 0,
        "client_latency_gt_5s": int((lat > 5000).sum()) if len(lat) else 0,
        "missing_csv": 0,
    })
...
if health_csv.exists():
    h = pd.read_csv(health_csv)
    if "success" in h.columns:
        health_fail_total = int((h["success"].astype(str) != "1").sum())
    else:
        health_fail_total = -1
else:
    health_fail_total = -1
...
clean = True
clean &= len(summary) == 3
clean &= bool((summary["missing_csv"] == 0).all())
clean &= bool((summary["success_rate_pct"] >= 99.999).all())
clean &= bool((summary["client_latency_gt_1s"] == 0).all())
clean &= bool((summary["client_latency_gt_5s"] == 0).all())
clean &= bool((summary["client_max_ms"] <= 500).all())
clean &= health_fail_total == 0
clean &= warmup_fail_total == 0

validity_txt = summary_dir / "run_validity.txt"
with open(validity_txt, "w") as f:
    f.write(f"clean_normal_candidate={clean}\n")
    f.write(f"health_fail_total={health_fail_total}\n")
    f.write(f"warmup_fail_total={warmup_fail_total}\n")
    f.write("criteria=success_rate_100pct,no_client_latency_gt_1s,no_healthz_fail,no_warmup_fail,client_max_le_500ms\n")
